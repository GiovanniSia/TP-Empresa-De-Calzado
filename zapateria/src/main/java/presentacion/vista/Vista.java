...
public class Vista {
	...
	private String[] nombreColumnas = { "Nombre", "id Proveedores" };
	...
	private void initialize() {
		...
		tablaPersonas = new JTable(modelProducto);
	...
	public void llenarTabla(List<ProductoDTO> productoEnTablas) {
		this.getModelPersonas().setRowCount(0); // Para vaciar la tabla
		this.getModelPersonas().setColumnCount(0);
		this.getModelPersonas().setColumnIdentifiers(this.getNombreColumnas());

		for (ProductoDTO p : productoEnTablas) {
			String nombre = p.getNombre();
			int idProveedor = p.getIdProveedor();
			Object[] fila = { nombre, idProveedor };
