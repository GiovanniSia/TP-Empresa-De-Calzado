...
//	private final List<MaestroProductoDTO> carrito;
	...
	public ControladorBusquedaProductos(MaestroProducto maestroProducto, Stock stock, Sucursal sucursal,
			Carrito carrito, DetalleCarrito detalleCarrito) {
...
//				//cuando se clickea en la tabla, se actualiza el spinner
				vistaBusquedaProductos.getSpinnerCarrito()
						.setValue(productosEnCarrito.get(filaSeleccionada).getCantidad());
			}
...
//	public void realizarBusqueda(ActionEvent a) {
	public void realizarBusqueda() {
		...
		this.vistaBusquedaProductos.getModelTabla()
				.setColumnIdentifiers(this.vistaBusquedaProductos.getNombreColumnas());
		...
		if (this.clienteSeleccionado.getTipoCliente().equals("Mayorista")) {
			productosAproximados = this.maestroProducto.getMaestroProductoAproximado("Descripcion", txtNombre, "Talle",
					txtTalle, "PrecioMayorista", precioDesde, "PrecioMayorista", precioHasta);
		} else {
			productosAproximados = this.maestroProducto.getMaestroProductoAproximado("Descripcion", txtNombre, "Talle",
					txtTalle, "PrecioMinorista", precioDesde, "PrecioMinorista", precioHasta);
		}
		...
		for (StockDTO s : this.listaStock) {
			for (MaestroProductoDTO m : this.listaMaestroProducto) {
				if (m.getIdMaestroProducto() == s.getIdProducto() && idSucursal == s.getIdSucursal()
						&& esAptoParaVender(s, m) && m.getTipo().equals("PT")) {
					agregarATabla(s, m);
	...
	public void actualzarTablaCarrito() {
		...
		this.vistaBusquedaProductos.getModelTablaCarrito()
				.setColumnIdentifiers(this.vistaBusquedaProductos.getNombreColumnasCarrito());
	...
	public void quitarProductoDelCarrito(ActionEvent a) {
		...
		modificarCantStock(productoSeleccionado.getProducto(), productoSeleccionado.getStock().getIdStock(),
	...
	public void modificarCantDeCarritoDadoSpinner() {
		...
		// si el valor que se acaba de actualizar es el mismo al de la cantidad original
		// significa que no se hizo click sobre la tabla, no se desconto valor desde el
		// spinner
		...
		if (!laCantidadEsValida(valorDelSpinner, productoEnCarrito.getProducto().getIdMaestroProducto(),
				productoEnCarrito)) {
			JOptionPane.showMessageDialog(null, "La cantidad elegida no es valida");
			return;
		}

		// dado el spinner si el nuevo valor es menor que la cant del carrito significa
		// que se le resta al carrito (se le suma al stock original), si es mayor se le
		// suma uno al carrito (se le descuenta al stock original)
...
//		ControladorBusquedaCliente c = new ControladorBusquedaCliente(cliente);
	...
	public void armarVenta(ActionEvent a) {
		int resp = JOptionPane.showConfirmDialog(null, "Estas seguro que desea armar la venta?", "Armar venta",
		...
		if (resp == 0) {
			...
			JOptionPane.showConfirmDialog(null,
					"Venta armada con exito a las " + dtf.format(LocalDateTime.now()) + ".\n "
							+ "En espera de ser efectuada por un cajero",
					"Armar venta", JOptionPane.CLOSED_OPTION, JOptionPane.QUESTION_MESSAGE);
	...
	public void guardarCarrito() {
		...
		CarritoDTO carrito = new CarritoDTO(0, idSucursal, this.clienteSeleccionado.getIdCliente(), this.idVendedor,
				this.Preciototal, hora);
...
//		El id ser� el mismo para todos los sig prod
		...
		for (ProductoEnCarritoDTO compra : this.productosEnCarrito) {
			...
			DetalleCarritoDTO detalleCarrito = new DetalleCarritoDTO(0, ultIdCarrito, idProducto, idStock, cant,
					precio);
	...
	public boolean hayStockDisponible() {
		...
		for (StockDTO s : this.listaStock) {
			for (ProductoEnCarritoDTO compra : this.productosEnCarrito) {
				if (s.getIdProducto() == compra.getProducto().getIdMaestroProducto()
						&& s.getIdSucursal() == this.idSucursal) {
	...
	public void descontarDelStock() {
		for (StockDTO stock : this.listaStock) {
			for (ProductoEnCarritoDTO compra : this.productosEnCarrito) {
				if (stock.getIdProducto() == compra.getProducto().getIdMaestroProducto()
						&& stock.getIdSucursal() == this.idSucursal) {
